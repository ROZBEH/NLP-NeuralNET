#! /usr/bin/env python
# LSTM many to one structure for code switch point prediction task
# @author : Rouzbeh Shirvani    
__author__ = "Rouzbeh Shirvani"
__copyright__ = "Copyright 2017, The ESCALES Project"
import csv
import itertools
import operator
import numpy as np
...
tokenized_sentences = []
# Switch labels at each word label
switch_label = []
# Feature vector for each word
feat_vec = []
# One hot representation of the feature vector
feat_1hot = []
# all language labels that are available such as 'en', 'sw', ...
# List of the features in the feature vectors that we are going to have one hot representation of them
dic0 = {'en':0, 'sw':1, 'other':2, 'punc':4, 'mixed':3}
dic1 = {'init':0, 'en':1, 'sw':2, 'other':3, 'punc':5, 'mixed':4}
dic2 = {'init': 0, 'same':1, 'diff':2 }
# Looping through the interview files and reading each utterance and extracting the features related to each specific word
...
for inview in inview_split:
    ...
    if this_inview[0] == "":
        for i in range(3,len(this_inview)):
            ...
            for word in utter:
                feat = []
                # handling exceptions such as the empty utterances
                try:
                    features = word.split(',')
                    if features[3] != 'punc':
                        # adding features to the feat vector and put them into one vector that is readable for mathematical operation
...
# Create the training data and change them into vector form by using numpy
