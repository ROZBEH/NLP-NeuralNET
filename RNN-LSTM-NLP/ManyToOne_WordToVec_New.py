...
class RNNNumpy:
    ...
    def __init__(self, vector_dim, hidden_dim, label_dim = 6 , bptt_truncate = 6):
        ...
        self.W = np.concatenate((self.u,self.w) , axis = 1)
...
def bptt(self, x, y):
    ...
    delta_t = self.V.T.dot(delta_o) * (1 - (s[T-1] ** 2))
    # Backpropagation through time (for at most self.bptt_truncate steps)
    # The reason that we have (T-1) here is that in the full version we have t+1 for all
    # steps and since here we are working with the last t which is (T-1) thats the reason
    # this notation
    for bptt_step in np.arange(max(0, (T-1)-self.bptt_truncate), (T-1)+1)[::-1]:
        ...
        delta_t = self.W[:,label_dim:].T.dot(delta_t) * (1 - s[bptt_step-1] ** 2)
    #print dLdW
